...
def knot_hash_round(num_list, length_list, curr_pos, skip_size):
    ...
    for length in length_list:
        ...
        for pos in range(curr_pos, curr_pos + length):
            pos = pos % size
            subset.append(num_list[pos])
        subset = list(reversed(subset))
        for count, pos in enumerate(range(curr_pos, curr_pos+length)):
            pos = pos % size
            num_list[pos] = subset[count]
        curr_pos = ( curr_pos + length + skip_size ) % size
        skip_size += 1
    
    return curr_pos, skip_size

def solve1(sequence, size):
    num_list = []
...
# first element in the dense hash is the first sixteen elements of the
...
# Knot Hash is sixteen such numbers, the hexadecimal representation is
...
def convert_to_dense_hash(sparse_hash):
    dense_hash = []
    element = []
    for x in range(256):
        if x % 16 == 0 and len(element) > 0:
            dense_hash.append(reduce(lambda x,y: x^y, element))
            element = [sparse_hash[x]]
        else:
            element.append(sparse_hash[x])
    dense_hash.append(reduce(lambda x,y: x^y, element))
    return dense_hash
...
def knot_hash(ascii_string):
    ...
    for num in range(256):
        num_list.append(num)
    for char in ascii_string.strip():
        d_hex = ord(char)
