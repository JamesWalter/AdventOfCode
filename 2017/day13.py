...
def get_severity(depth, layer_range, delay=0):
    time = depth + delay
    round_trip = (layer_range - 1) * 2
    if time % round_trip == 0:
...
def build_firewall(filename):
    with open(filename, "r") as inputfile:
        ...
        for line in inputfile:
            fields = line.split(': ')
            layers[int(fields[0])] = int(fields[1].rstrip())
    return layers

def solve1(firewall):
    total_sev = 0
    depths = firewall.keys()
    depths.sort()
    for depth in depths:
        sev = get_severity(depth, firewall[depth])
        if sev is not None:
            total_sev += sev
    return total_sev
...
def solve2(firewall):
    ...
    depths = firewall.keys()
    depths.sort()
    fw_size = len(firewall)
    while True:
        delay += 1
        for count, depth in enumerate(depths):
            sev = get_severity(depth, firewall[depth], delay)
            if sev is not None:
                break
